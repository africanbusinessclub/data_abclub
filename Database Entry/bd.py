...
# Définition des variables dossiers_racines et prefixes
dossiers_racines = [
    "G:\\Drive partagés\\Général 2023-2024\\AMID",
    "G:\\Drive partagés\\Général 2023-2024\\Administratif",
    "G:\\Drive partagés\\Général 2023-2024\\Compte rendu des réunions",
...
@app.route('/')
def index():
    return render_template('index.html', dossiers_racines=prefixes)

# Route pour la recherche de fichiers
@app.route('/search', methods=['POST'])
def search():
    # Récupérer le terme de recherche depuis la requête POST
    search_term = request.form.get('search_term')

    # Liste pour stocker les chemins des fichiers correspondants à la recherche
    matching_files = []
    ...
    for dossier in dossiers_racines:
        ...
        for root, _, files in os.walk(dossier):
            for file in files:
                # Vérifier si le terme de recherche est dans le nom du fichier
                if search_term.lower() in file.lower():
                    matching_files.append(os.path.join(root, file))

    # Retourner les chemins des fichiers correspondants en tant que réponse JSON
